...
def start_game_setup(test_db: Session):
    ...
    user1 = User(
        name="Host",
        email="starthost@example.com",
        hashed_password=get_password_hash("hostpass"),
    )
    user2 = User(
        name="Player",
        email="startplayer@example.com",
...
def test_start_game_isolated(test_db: Session, start_game_setup):
    ...
    with patch(
        ...
        "app.api.endpoints.game.send_game_state_update", new_callable=AsyncMock
    ) as mock_send_state, patch(
        "app.api.endpoints.game.start_round_timer", new_callable=AsyncMock
    ) as mock_timer:
