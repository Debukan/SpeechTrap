...
async def create_room(
    ...
    players = [PlayerResponse(id=new_player.id, name=current_user.name)]
    ...
    room_response = RoomResponse(
        ...
        player_count=1,
        current_word_id=(
            new_room.current_word_id if hasattr(new_room, "current_word_id") else None
        ),
        is_full=False,
        players=players,
    )
...
async def join_room_by_code(
    ...
    async def broadcast_player_joined():
        await manager.broadcast(
            room_code, {"type": "player_joined", "player": player_data}
        )

    background_tasks.add_task(broadcast_player_joined)

    return {
        "message": f"Пользователь {user.name} успешно присоединился к комнате {room.code}"
    }
...
async def delete_room(
    ...
    if not room.players or len(room.players) == 0:
        raise HTTPException(
            status_code=400, detail="Невозможно определить создателя комнаты"
        )

    first_player = room.players[0]
    if first_player.user_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Только создатель комнаты может её удалить"
        )
...
async def leave_room(
    ...
    if (
        room_code in manager.active_connections
        and current_user.id in manager.active_connections[room_code]
    ):
